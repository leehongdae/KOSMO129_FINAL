...
<script>
	$(document).ready(function(){
		$(document).on("click", "#updatebtn", function(){
			var data = $(this).parent().parent().find("td:eq(0)").text();
			location.href="gtEmployeeSelect.gos?genum="+data;
		});
	});
	$(function() {
		performSearch();
	...
	function updateTable(data) {
		...
		$.each(data, function(index, item) {
			...
			row.append($("<td>").html("<input type='button' id='updatebtn' value='선택'>"));
				...
				<div class="wrapper_i01">
					<div class="content_i01">
						<div class="section_i01">
							<div>
								<img src="\resources\goats_icon\my.png" alt="img"> <span>조직도
									인원 현황</span>
							</div>
							<div class="selectArea">
								<select id="searchGedept" class="">
									<!-- 구분 -->
								...
								</select> <input type="text" id="searchGename" placeholder="이름입력">
								<button onclick="performSearch()">검색</button>
							</div>
						</div>
					</div>
					<div class="content_i01" style='overflow: scroll; height: 750px'>
						<table>
							<!-- js로 오름, 내림 처리 -->
							<tr>
								<td>사번</td>
							...
							<tbody id="dataBody">
								<c:forEach var="list" items="${aList}">
									<tr>
										...
										<td>${list.geemail}</td>
										<td>${list.gehp}</td>
										<td><input type="button" id="updatebtn" value="선택"></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
